...
def sliceim(im):
    ...
    for i in range(numslice-1):
        for j in range(numslice-1):
            ...
            var=np.append(var,np.std(imslice)+np.fabs(np.mean(imslice)))
    p16=np.percentile(var,16)
    p84=np.percentile(var,84)
    med=np.median(var)
    if (np.max(var) > med+8*(med-p16)) & (p84-med <= 3*(med-p16)):
        return True
...
for memory in listname:
    ...
    #for index in [0]:
        ...
        if (len(np.shape(imcube))>2) | (np.max(imcube)>0):
            ...
            print 'orig ',out2sigma
            # If too many images are selected then perhaps we  are dealing with noisy maps. I only output the first 10 and the last 10 images. 
            if len(out2sigma)> 40:
                idsort=np.argsort(stdpcablur[out2sigma])
                min10=out2sigma[idsort[0:10]]
                max10=out2sigma[idsort[-10:]]
                out2sigma=np.append(min10,max10)
                out2sigma=out2sigma[np.argsort(out2sigma)]
                print 'Two many outputs. Reducing to only first 10 and the last 10 images'
            print 'last ', out2sigma
            # Fill in the blanks between the out2sigma IDs that might have been missed
            if (len(out2sigma) > 0):
                if  out2sigma[0] > 0:
                    np.insert(out2sigma,0,out2sigma[0]-1)
                    print 'last 1', out2sigma
                if  out2sigma[-1] < len(imlistblur)-1:
                    out2sigma=np.append(out2sigma,out2sigma[-1]+1)
                    print 'last 2', out2sigma
                arr2=out2sigma[1:]-out2sigma[0:-1]
                id=np.where(arr2 != 1)[0]
                if len(id) > 0:
                    out2sigma=np.insert(out2sigma,id+1,out2sigma[id]+1)
                    print 'last 3', out2sigma
            ...
            if   (len(indday) <= 20):# & (len(out2sigma) <=5):
